...
internal sealed class GameStore
{
    ...
    /// <summary>null = detection reelle. true/false = reponse imposee, pour les tests seulement.
    /// Les deux sens sont necessaires : "ferme" rend les tests d'ecriture independants de ce que
    /// l'utilisateur a ouvert, et "ouvert" est le SEUL moyen d'exercer le transit par la forme GUID,
    /// donc les garde-fous qui n'existent que pour lui. Un sceau a sens unique laissait passer une
    /// mutation qui supprimait l'un d'eux sans faire tomber un seul test.</summary>
    internal static bool? ForceLaunchBoxRunning;

    public static bool IsLaunchBoxRunning()
    {
        if (ForceLaunchBoxRunning.HasValue) return ForceLaunchBoxRunning.Value;
