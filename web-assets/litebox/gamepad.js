/* ============================================================================
   LaunchBox Web — manette (Gamepad API / XInput)

   Pilote la MÊME navigation que le clavier via window.LBW.nav(cmd, repeat).
   ...
   Mapping "standard" (manette Xbox / XInput sous Edge/Chrome Windows) :
     A(0)=select · B(1)=retour · Y(3)=change de zone · Start(9)=menu ·
     LB(4)=média précédent · RB(5)=média suivant · L2(6)=pgup · R2(7)=pgdn ·
     croix directionnelle (12-15) + stick gauche (axes 0/1) = directions.

   Réglages : window.LBW.config.input.gamepad
              {enabled, deadzone, repeatDelayMs, repeatRateMs, repeatRateMinMs,
               accelMs, triggerThreshold}.
   Chargé APRÈS app.js (a besoin de window.LBW.nav). Script classique (file://-safe).
   Calqué sur BigBoxWeb/web/engine/gamepad.js.
   ========================================================================== */
(function () {
  "use strict";
  function cfg() {
    return (window.LBW && window.LBW.config && window.LBW.config.input &&
            window.LBW.config.input.gamepad) || {};
  }
  function nav(cmd, repeat) { if (window.LBW && window.LBW.nav) window.LBW.nav(cmd, repeat); }
  ...
  function loop() {
    ...
    if (gp) {
      ...
      edge(gp, 0, "select");   // A
      edge(gp, 1, "back");     // B
      edge(gp, 3, "zone");     // Y → change de zone (gauche / centre / droite)
      edge(gp, 9, "menu");     // Start → menu système
