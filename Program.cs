...
// Monitor Profiles driver: --monitor <probe|state|list|apply|restore|forget|capture|hdr> (see MonitorProbe).
// --monitor-probe stays as the read-only shorthand it has always been.
if (args.Contains("--monitor") || args.Contains("--monitor-probe"))
    return LbApiHost.Host.Monitors.MonitorProbe.Run(args);
...
if (args.Contains("--selftest-entry-saves"))
    return LbApiHost.Tools.EntrySaveSelfTest.Run(args);
...
if (args.Contains("--selftest-http"))
    return LbApiHost.Host.Web.HttpSelfTest.Run();
if (args.Contains("--selftest-romm"))
    return LbApiHost.Host.Romm.RommSelfTest.Run();
...
if (args.Contains("--hid-dump"))
{
    ...
    {
        ("HIDSHARP", LbApiHost.Host.Rules.Hid.HidInfoCache.HidSharpInfo),
        ("DS4LIB", LbApiHost.Host.Rules.Hid.HidInfoCache.Ds4Info),
        ("XINPUTLIB", () => LbApiHost.Host.Rules.Hid.HidInfoCache.XInputInfo("")),
        ("DINPUTLIB", LbApiHost.Host.Rules.Hid.HidInfoCache.DInputInfo),
        ("SDLLIB", LbApiHost.Host.Rules.Hid.HidInfoCache.SdlInfo),
        ("SDLLIB-NoRI", LbApiHost.Host.Rules.Hid.HidInfoCache.SdlNoRIInfo),
        // BLUETOOTHLIB last and only with --hid-bt: an inquiry can take seconds.
    })
    { Console.WriteLine($"{name}:"); Console.WriteLine(dump()); }
    if (args.Contains("--hid-bt"))
    { Console.WriteLine("BLUETOOTHLIB:"); Console.WriteLine(LbApiHost.Host.Rules.Hid.HidInfoCache.BtInfo()); }
    return 0;
}
