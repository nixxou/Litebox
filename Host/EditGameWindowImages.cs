...
internal sealed partial class EditGameWindow
{
    ...
    // Le verdict de collision, calcule UNE fois pour la duree de la fenetre. L'ancienne version
    // parcourait les 5645 jeux a CHAQUE fichier ajoute ou telecharge ; la collision, elle, ne
    // change pas pendant qu'on ajoute des images.
    private bool? _imgCollision;

    private bool ImgNameCollision(string plat, string idStr, string sani)
    {
        if (_imgCollision.HasValue) return _imgCollision.Value;
        int? dbid = null;
        try { dbid = _editGames.Count > 0 ? _editGames[0].LaunchBoxDbId : null; } catch { }
        IEnumerable<IGame>? all = null;
        try { all = PluginHelper.DataManager?.GetAllGames(); } catch { }
        // La regle vit dans MediaCollision, seule definition : celle-ci en avait une quatrieme, qui
        // ignorait le DatabaseID et separait donc 68 groupes de titres partages sur 70 — dont 59
        // ou les deux fiches sont le MEME jeu et doivent partager leurs fichiers.
        _imgCollision = MediaCollision.NeedsGuidForm(plat, idStr, sani, dbid, MediaCollision.From(all));
        return _imgCollision.Value;
    }
