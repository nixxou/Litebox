// Verifies the unlock PIN against BigBox's own <LockPin> in Data\BigBoxSettings.xml — the same
// single credential LiteBox uses. The blob is Rijndael-256 / CBC / PKCS7 with a fixed LaunchBox
// key+seed (recovered by LiteBox — see Host/Data/LbSettingsCrypto); BouncyCastle (shipped in Core)
// does the 256-bit-block cipher .NET can't. A 3-strike lockout lives in PinLockout.
...
namespace LiteBoxParental
{
    internal static class PinVerify
    {
        ...
        /// <summary>The clear 4-digit PIN BigBox has set, or "" when none / unreadable.</summary>
        public static string Current()
        {
            try
            {
                ...
                var path = Path.Combine(core, "..", "Data", "BigBoxSettings.xml");
