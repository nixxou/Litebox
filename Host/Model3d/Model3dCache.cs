...
internal static class Model3dCache
{
    ...
    public const int BakerVersion = 16;   // v16: Saturn/Sega CD fall back from the long box to a plain jewel case when the front art fits that shape better (Model3dAutoJewelCase) — the decision is made in the builder, so unlike the Genesis fix it does NOT move the key on its own (v15: a forced ModelSizeString is normalised onto the unit box, so LB's Genesis/Master System defaults ("5;7.165;1" — inches) stop baking a model seven units tall in front of the camera, v14: art resolves ONCE, through MediaResolver with the game's id -> the game cache answers it (fast validation), GUID-form images become visible, and an image in a type folder is no longer read as a region, v13: Dreamcast Auto-Detect picks its spine COLOUR by measuring the artwork, v12: auto version follows the front art's region, v11: double-jewel strips keep the game scan, v10: doubleSided through the GLB)
