...
internal static class MediaRenameSelfTest
{
    ...
    public static int Run()
    {
        ...
        try
        {
            ...
            failures += RegionPriorityBeatsAlphabetical(root);
    ...
    private static int LaunchBoxOnlyFoldersAreInvisible(string root)
    {
        ...
        {
            int f = 0;
            // Un dossier nomme "01-North America" est un dossier comme un autre : rien ne l ecarte
            // du parcours. Un saut avait ete ajoute ici pour des jonctions "NN-<Region>" ; ce design
            // est abandonne, et le saut avec lui. Son NOM n etant pas une region connue, il passe
            // simplement apres celles-ci dans l ordre — mais il est bien visite.
        f += Check("aucun nom de dossier n'est exclu du parcours",
    ...
    /// <summary>Le SEUL endroit ou LiteBox s'ecarte volontairement de LaunchBox.
    ///
    /// La regle d'appartenance reste la sienne — nom de fichier, toute profondeur, nom de dossier
    /// indifferent. L'ORDRE de descente, lui, suit nos priorites de region. LaunchBox ne sait
    /// ordonner que par nom de dossier : avec "Europe" et "North America" il prend Europe, nous
    /// celui que la priorite designe. Les deux peuvent donc montrer un manuel different pour un meme
    /// jeu — c'est assume, pas subi.
    ///
    /// Le test choisit deux regions dont l'ordre alphabetique CONTREDIT l'ordre de priorite, sinon
    /// il passerait sans rien prouver.</summary>
    private static int RegionPriorityBeatsAlphabetical(string root)
    {
        string dir = Fresh(root, "Manuals", null);
        var order = MediaResolver.RegionOrderForTest();
        int rNa = order.FindIndex(x => string.Equals(x, "North America", StringComparison.OrdinalIgnoreCase));
        int rEu = order.FindIndex(x => string.Equals(x, "Europe", StringComparison.OrdinalIgnoreCase));
        Directory.CreateDirectory(Path.Combine(dir, "Europe"));
        Directory.CreateDirectory(Path.Combine(dir, "North America"));
        Touch(Path.Combine(dir, "Europe"), $"{Old}-01.pdf");
        Touch(Path.Combine(dir, "North America"), $"{Old}-01.pdf");

        string was = MediaResolver.SwapRootForTest(Case(root));
        try
        {
            // "Europe" trie avant "North America" : si la priorite place North America devant, le
            // resultat prouve que ce n'est pas l'alphabet qui a decide.
            string expected = (rNa >= 0 && (rEu < 0 || rNa < rEu)) ? "North America" : "Europe";
            return Check($"la priorite de region l'emporte sur l'alphabet (attendu : {expected})",
                MediaResolver.Manual(Plat, Id, Old) == Path.Combine(dir, expected, $"{Old}-01.pdf"));
        }
        finally { MediaResolver.SwapRootForTest(was); }
    }
