...
internal static class RulesPanel
{
    ...
    private static Control BuildGeneral(float dpiS)
    {
        ...
        Row(ModulePanelKit.Caption(
            ...
            + "the next lands. Available today: Prefix and Suffix (prepend / append), Change exe, Change rom path (relocation through prioritized folders), Replace (search/replace on the line, literal or regex, with an extraction-variables system), and Replace in file (rewrite a config file right before launch), Create file (write a file — path and content take variables), HID device detector (scans the plugged devices through the same libraries emulators read and appends one argument per matched device), Copy file (stage matching file arguments onto fast storage before launch, optional delete on exit), Use file content (a pointer-file argument is replaced by its content), and Monitor profile (pick the monitor profile for matching launches — above the stored assignments, below the manual one-shot; inert while the Monitor Profiles module is off), and Run C# script (Roslyn: four slots (transform real/example, before launch, after exit) with the line, the game/emulator objects, HID queries and monitor profiles in scope; documentation and examples in the rule dialog), and Run AHK script (same four-slot model through the AutoHotkey v1.1 exe LaunchBox ships — or your own v2 — with a resident mode for hotkeys during the game).", dpiS, 620), S(48), 18);
