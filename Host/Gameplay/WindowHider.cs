...
{
    // EnumWindows/IsWindowVisible/GetWindowThreadProcessId/GetClassName are already declared (and the
    // top-level-window enumeration loop already written) by Host.Diag.WinScan, shared with the RenderProbe
    // diagnostic and SmartCapture — Hide() below reuses WinScan.AllTopLevelWindows() instead of a third
    // copy of the same P/Invoke set and loop. Only the checks WinScan.Win doesn't carry (owner-popup,
    // tool-window style) still need their own declarations here.
    ...
    public static void Hide(int keepPid)
    {
        lock (_lock)
        {
            ...
            try
            {
                ...
                // top-level windows only) — only the owner-popup/tool-window/title-length/class checks below
                // are specific to what Hide() needs to exclude.
