...
internal static class LbModules
{
    ...
    public static readonly Info[] Catalog =
    {
        ...
        new(LbModule.Rules, "rules", "Launch rules",
            "BigBoxProfile's probes & actions, native: ordered rules attached to an EMULATOR rewrite the command line right before the spawn, guarded by filters — per-game targeting via marker arguments in the game's custom parameters, stripped before the emulator sees them. Ported action by action — today: Prefix, Suffix, Change exe, Change rom path, Replace (with a variables system), Replace in file. Off: launches run untouched.", false, true),
    };
