...
internal sealed partial class EditGameWindow
{
    ...
    /// <summary>(Re)builds a Controller combo column's choices + the shared display↔id map from the
    /// catalog. <paramref name="gridToPreserveFrom"/> (optional): also keep any value already used by a
    /// cell in that grid (unknown ids shown raw) so those cells keep rendering — the single-game grid
    /// needs this (a game can carry a raw id no longer in the catalog); the multi-select matrix grid never
    /// has pre-existing raw values to preserve, so it passes null.</summary>
