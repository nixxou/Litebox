...
internal sealed partial class EditGameWindow
{
    ...
    // The RE'd SupportLevel mapping lives with the controller catalog window (Host/Controllers), which
    // needs it too — these keep the local names this file's many call sites already use.
    private static readonly string[] SupportDisplay = Controllers.ControllerSupport.Display;
    private static string SupportToDisplay(string? level) => Controllers.ControllerSupport.ToDisplay(level);
    private static string? SupportToLevel(string? display) => Controllers.ControllerSupport.ToLevel(display);
    ...
    {
        // One implementation, two entry points (the other is Tools > Manage in the main menu):
        // Host/Controllers/ManageControllersWindow. Its catalog edits can move the ground under this
        // page, so the controller column and the support grid are rebuilt on every change.
        using var w = new Controllers.ManageControllersWindow(_readOnly);
        w.OnCatalogChanged += () => { RefreshControllerColumn(); LoadControllerSupport(); };
        w.ShowDialog(this);
    }
