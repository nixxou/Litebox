...
/// <summary>Un groupe de dossiers traites ensemble, et la nature du type de media qui les porte.
/// <see cref="Flat"/> distingue les manuels et les musiques du reste : chez eux aucun sous-dossier
/// n'est impose par LaunchBox, donc tout sous-dossier est un rangement libre, on y descend a toute
/// profondeur, et un fichier peut porter le nom nu du jeu sans numero. Pour les images et les
/// videos un sous-dossier signifie une region ou un type, et les niveaux ne se melangent pas.
/// Herite de List&lt;string&gt; pour que tout ce qui parcourait deja une unite continue de marcher.</summary>
internal sealed class MediaUnit : List<string>
{
    public readonly bool Flat;
    public MediaUnit(bool flat) { Flat = flat; }
}
...
internal static class GameMediaRenamer
{
    ...
    public static List<MediaUnit> Units(string lbRoot, string platform)
    {
        ...
        if (Directory.Exists(images))
            ...
            {
                var unit = new MediaUnit(flat: false) { typeDir };
        ...
        {
            var group = new MediaUnit(flat: false) { videos };
            group.AddRange(SafeDirs(videos));
            units.Add(group);
        }

        // Manuels et musiques : LaunchBox y descend a TOUTE profondeur et se moque du nom des
        // dossiers (mesure). Ne balayer que la racine laissait donc derriere, lors d'un renommage,
        // des fichiers que LaunchBox affichait — le jeu perdait son manuel.
        foreach (var kind in new[] { "Manuals", "Music" })
        {
            string dir = Path.Combine(lbRoot, kind, plat);
            if (!Directory.Exists(dir)) continue;
            var unit = new MediaUnit(flat: true) { dir };
            // Les points d'analyse sont ecartes ICI, pas a la resolution : LaunchBox les TRAVERSE
            // pour choisir un manuel (mesure), donc l'affichage doit les traverser aussi. Mais un
            // renommage qui les suivrait verrait le meme fichier physique par deux chemins et le
            // renommerait deux fois.
            try
            {
                unit.AddRange(Directory.EnumerateDirectories(dir, "*", SearchOption.AllDirectories)
                    .Where(d => { try { return (new DirectoryInfo(d).Attributes & FileAttributes.ReparsePoint) == 0; }
                                  catch { return false; } }));
            }
            catch { }
            units.Add(unit);
        ...
        bool append = false, bool sharedSource = false)
    {
        ...
        foreach (var unit in Units(lbRoot, platform))
        {
            ...
            else if (plain.Count > 0 && !string.Equals(fromSani, toSani, StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (var (path, num) in plain.OrderBy(p => p.Num))
                {
                    // Un fichier nu reste nu. Lui attribuer un numero le ferait passer DEVANT
                    // les fichiers numerotes du meme jeu, donc changer le manuel affiche.
                    if (num == Unnumbered)
                    {
                        moves.Add(new MediaMove(path, PlainPath(path, toSani, Unnumbered), sharedSource));
                        continue;
                    }
    ...
    internal static HashSet<int> TakenNumbers(List<string> unit, Guid id, string sani, MediaNameForm form)
    {
        ...
        foreach (var dir in unit)
            foreach (var file in SafeFiles(dir))
            {
                ...
                // Un fichier au nom nu n'occupe aucun numero : l'ajouter reserverait un creneau
                // qui n'existe pas.
                else if (sani.Length > 0 && TryPlain(name, sani, out int n2) && n2 != Unnumbered) taken.Add(n2);
            }
        return taken;
    }
    ...
    internal static string PlainPath(string source, string sani, int num)
        => Path.Combine(Path.GetDirectoryName(source)!,
                        num == Unnumbered ? $"{sani}{Path.GetExtension(source)}"
                                          : $"{sani}-{num:D2}{Path.GetExtension(source)}");
    ...
    /// <summary>Les deux titres visent-ils le MEME nom de fichier ? A calculer avant tout le reste :
    /// si la reponse est oui, il n'y a rien a faire pour aucun media, et surtout pas de passage par
    /// la forme GUID — LaunchBox, qui lit encore l'ancien titre, cherche deja le meme fichier que
    /// nous.
    ///
    /// Sanitize n'est PAS injectif, et pas seulement sur des cas tordus : l'apostrophe devient "_"
    /// (317 titres de la bibliotheque en portent une), les "_" consecutifs sont reduits a un seul, et
    /// le resultat est trimme. "Disney's Aladdin" et "Disney_s Aladdin" produisent donc le meme
    /// fichier, "A::B" et "A_B" aussi. La comparaison ignore la casse parce que le systeme de
    /// fichiers l'ignore : renommer un jeu de "aladdin" en "Aladdin" designe le meme fichier, et le
    /// deplacer ne changerait que l'affichage de son nom.</summary>
    internal static bool SameTargetName(string oldTitle, string newTitle)
        => string.Equals(MediaResolver.Sanitize(oldTitle ?? ""), MediaResolver.Sanitize(newTitle ?? ""),
                         StringComparison.OrdinalIgnoreCase);

    internal static bool TryPlain(string nameNoExt, string sani, out int num, bool allowUnnumbered = false)
    {
        num = 0;
        int dash = nameNoExt.LastIndexOf('-');
        if (dash > 0
            && string.Equals(nameNoExt.Substring(0, dash), sani, StringComparison.OrdinalIgnoreCase)
            && int.TryParse(nameNoExt.Substring(dash + 1), out num)) return true;
        // Le nom NU, valable pour les manuels et les musiques seulement. Unnumbered le represente :
        // le fichier n'occupe aucun numero et n'en recevra pas, sans quoi un renommage le ferait
        // passer devant les fichiers numerotes du meme jeu et changerait le manuel affiche.
        if (allowUnnumbered && string.Equals(nameNoExt, sani, StringComparison.OrdinalIgnoreCase))
        { num = Unnumbered; return true; }
        num = 0;
        return false;
    }

    /// <summary>Marque un fichier au nom nu "&lt;titre&gt;.ext", qui n'a pas de numero et ne doit pas
    /// en recevoir.</summary>
    internal const int Unnumbered = -1;
