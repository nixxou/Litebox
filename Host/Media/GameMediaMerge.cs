...
internal static class GameMediaMerge
{
                                 ...
                                 Guid destId, string destTitle, DupEngineMode mode, double threshold)
    {
        ...
        foreach (var unit in GameMediaRenamer.Units(lbRoot, platform))
            ...
            {
                var here = FilesOf(dir, to, destId, unit.Flat).ToList();
                destByDir[dir] = here;
        ...
        foreach (var unit in GameMediaRenamer.Units(lbRoot, platform))
        {
            ...
            foreach (var dir in unit)
                // Un fichier nu de la source recoit un numero en arrivant : il rejoint une
    ...
    /// <summary>Les fichiers d'un jeu dans un dossier. <paramref name="flat"/> vient de l'unite :
    /// pour les manuels et les musiques, le nom NU "&lt;titre&gt;.ext" appartient aussi au jeu. Sans ce
    /// drapeau, la fusion d'un combine ne voyait pas ces fichiers : pas deplaces en consolidation,
    /// ni orphelins ni effaces quand les DatabaseID different — ils restaient derriere en silence —
    /// et absents de l'inventaire CRC de la destination, ou un doublon octet-identique entrant
    /// passait donc le filtre.</summary>
    private static IEnumerable<string> FilesOf(string dir, string sanitizedTitle, Guid id, bool flat)
        => Safe(dir).Where(f =>
        {
            string n = Path.GetFileNameWithoutExtension(f);
            if (id != Guid.Empty && GameMediaRenamer.TryGuid(n, id, out _, out _)) return true;
            return GameMediaRenamer.TryPlain(n, sanitizedTitle, out _, flat);
        }).ToList();
