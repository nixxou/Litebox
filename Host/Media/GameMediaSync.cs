...
/// <summary>Ce que l utilisateur a decide face a une collision de titres. Auto = pas de
/// dialogue (pas de rival, ou appel sans interface) : les regles automatiques tranchent.</summary>
public enum CollisionChoice { Auto, Merge, SplitGuid, DeleteOurs, LeaveMedia, CancelRename }
...
internal static class GameMediaSync
{
    ...
    /// <summary>Vrai quand la mecanique des medias peut agir — le dialogue de collision ne se
    /// montre pas quand rien ne suivra (lecture seule, desactive, pas de magasin).</summary>
    public static bool CanTouchMedia => _store != null && !_store.ReadOnly && Enabled;
    ...
    public static void OnTitleChanged(IGame game, string oldTitle, string newTitle,
                                       CollisionChoice choice = CollisionChoice.Auto)
    {
        try
        {
            if (game == null) return;
            ...
            if (!Enabled) return;
            // Les deux choix qui ne touchent a rien. CancelRename est defensif : l appelant
            // n aurait pas du poser le titre, mais s il l a fait, ne rien casser derriere.
            if (choice == CollisionChoice.LeaveMedia || choice == CollisionChoice.CancelRename) return;
            ...
            // Le dialogue a tranche : son choix REMPLACE les regles automatiques. Merge force la
            // fusion filtree meme entre DatabaseID differents — « copier en mode fusion » est un
            // des trois choix de la spec, quelle que soit la preuve.
            if (choice == CollisionChoice.Merge) merge = rival != null;
            ...
            // "rival != null && !merge" basculait aussi en GUID quand l'un des deux n'a PAS de
            // DatabaseID : deux jeux non identifies etaient donc traites comme surement differents.
            // MediaCollision exige deux DatabaseID presents ET differents, et c'est desormais la
            // seule definition de cette question dans tout le code.
            ...
            if (choice == CollisionChoice.Merge) collision = false;
            else if (choice == CollisionChoice.SplitGuid) collision = rival != null;
            ...
            // « Effacer nos fichiers source » : le renommage se fait, les medias meurent. Deux
            // exceptions, toutes deux parce qu effacer ne se repare pas : les nominatifs d un
            // titre encore porte par un TIERS (ce sont aussi ses fichiers), et tout fichier
            // qu un chemin stocke designe encore.
            if (choice == CollisionChoice.DeleteOurs)
            {
                string fromSani = MediaResolver.Sanitize(oldTitle);
                var pinnedDel = PinnedMedia.For(lbRoot, platform);
                ...
                foreach (var unit in GameMediaRenamer.Units(lbRoot, platform))
                    foreach (var dir in unit)
                    {
                        ...
                        foreach (var f in files)
                        {
                            ...
                            bool ours = GameMediaRenamer.TryGuid(n, id, out _, out _)
                                     || (!sharedSource && GameMediaRenamer.TryPlain(n, fromSani, out _, unit.Flat));
                            if (ours && !PinnedMedia.IsPinned(pinnedDel, f)) doomed.Add(f);
                        }
                    }
                MediaCleanup.Delete(doomed, "collision : sources effacees sur choix utilisateur");
    ...
    /// <summary>Les jeux d'une plateforme : source unique pour toutes les questions de collision.</summary>
    internal static IEnumerable<IGame> PlatformGames(string platform)
    {
        try { return PluginHelper.DataManager?.GetPlatformByName(platform)?.GetAllGames(true, true) ?? Array.Empty<IGame>(); }
